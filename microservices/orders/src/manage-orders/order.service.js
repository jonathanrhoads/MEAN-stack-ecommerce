...
const productQueries = require('./order.query');
...
class OrderService {
    ...
    async createProduct({ productId, count, status }) {
        try {
            ...
            const { insertId } = await this.databaseService.query(this.INSERT_CREATE, [productId, count, status]);
    ...
    async updateOrder({ productId, count, status }, orderId) {
        try {
            // Insert into Products
            const { updateId } = await this.databaseService.query(this.UPDATE_ORDER, [productId, count, status, orderId]);
    ...
    async deleteOrder(productId) {
        try {
            // Insert into Products
            const { deleteId } = await this.databaseService.query(this.DELETE_ORDER, [productId]);
